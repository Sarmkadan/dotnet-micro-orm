...
public static class BatchOperationsTestsJsonExtensions
{
    ...
    /// <returns>A deserialized <see cref="BatchOperationsTests"/> instance, or null if the JSON is empty or whitespace</returns>
    ...
    public static BatchOperationsTests? FromJson(string json)
    {
        ...
        return JsonSerializer.Deserialize<BatchOperationsTests>(json, _jsonSerializerOptions);
    }
    ...
    private static void JsonContextModifier(JsonTypeInfo typeInfo)
    {
        ...
        if (typeInfo.Type == typeof(BatchOperationsTests) ||
            typeInfo.Type == typeof(BatchOperationsUpsertTests))
        {
